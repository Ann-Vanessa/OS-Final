...
// The thread pool design typically involves workers waiting for tasks to be enqueued. 
// Once a task is available, a worker thread will dequeue it, execute the function specified in the q_node, and then look for more tasks.
...
struct thread_pool{
    ...
    int q_head, q_tail; // for managing the queue's front and rear positions
    int max_workers; // maximum number of worker threads
    int TASK_Q_MAX; // maximum size of the task queue
