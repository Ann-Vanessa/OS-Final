...
// //
// void client_send(int fd, const char *filename) {
//     char buf[MAXBUF], hostname[MAXBUF];
    
//     gethostname_or_die(hostname, MAXBUF);
    
//     /* Form and send the HTTP request */
//     sprintf(buf, sizeof(buf), "GET %s HTTP/1.1\n", filename);
//     sprintf(buf,  "%shost: %s\n\r\n", buf, hostname);
...
//     char buf[MAXBUF];  
//     int n = readline_or_die(fd, buf, MAXBUF);
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        fprintf(stderr, "Usage: %s <host> <port> <no_of_requests> <filename1> ... <filenameN>\n", argv[0]);
        exit(EXIT_FAILURE);
    }

    const char *host = argv[1];
    int port = atoi(argv[2]);
    int no_of_requests = atoi(argv[3]);

    for (int i = 0; i < no_of_requests; i++) {
        int clientfd = open_client_fd_or_die(host, port);
        client_send(clientfd, argv[4 + i]);
        client_print(clientfd);
        close_or_die(clientfd);
    }
